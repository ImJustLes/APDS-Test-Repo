...
/*app.post('/esignup', async (req, res) => {
    ...
    try {
      const client = await database.database_connect();
      ...
      if (!existingUser) {
        if (passwordRegex.test(userModel.password) && emailRegex.test(userModel.email)) {
          ...
          const result = await collection.insertOne(userModel);
          res.status(201).send({ message: 'Employee created successfully', result });
        } else {
          res.status(401).json({ message: 'Invalid email and/or password format' });
        }
      } else {
...
app.post('/elogin', async (req, res) => {
    try {
      ...
      const token = jwt.sign(
        ...
        'your_jwt_secret', // Replace with a secure secret or use environment variables
        { expiresIn: '1h' }
      );
...
app.get('/GetPayments', async (_, res) => {
...
app.post('/make_payment',brute.prevent,
    ...
    try{
        ...
        const result = await Make_Payment.make_payment({ loginUserID ,amount, currency, SWIFT })
        res.status(201).json({message: 'Payment made successfully', result}) //return result of insersion
    }
...
//     }
//     try {
//         // Get the loginUserID from the request parameters
//         const { loginUserID } = req.params;

//         // Fetch payment details from the database
//         const result = await Make_Payment.viewPayments(loginUserID);
...
//         res.status(200).json({ message: 'Payment details retrieved successfully', payments: result });
//     } catch (error) {
//         console.error('Error when attempting to view payments:', error);
//         res.status(500).json({ message: 'Internal Server Error' });
//     }
// })
//#endregion
...
// app.get('/view_banking_details', checkAuthentication, async (_, res) =>{
...
//     }) 
    ...
    //#region Employee - View All Payments
//Update payment status
app.patch('/verify_payment/:payID',
    check('payID').isMongoId().withMessage('Invalid payment ID'),
    async (req, res) => { 
        try {
            const query = { payID: req.params.payID };
            console.log(query);
            const client = await database.database_connect();
            const db = client.db(databaseName);
            const paymentCollection = db.collection('Payments');

            // Update paymentStatus to "Confirmed" without deleting the document
            ...
            if (result.matchedCount === 0) {
                // If no document was found with the given payID
                return res.status(404).send('Payment not found');
            }

            res.status(200).send({ message: "Payment confirmed successfully", result });
        } catch (error) {
